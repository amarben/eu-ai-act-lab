...
export function EditRiskDialog({ isOpen, onClose, risk }: EditRiskDialogProps) {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      affectedStakeholders: risk.affectedStakeholders.join(', '),
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error?.message || 'Failed to update risk');
      }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Update the risk information</DialogDescription>
        </DialogHeader>
